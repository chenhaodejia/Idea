 ...
 * 
 */
public class SelfDiskCache1 implements Cache {
	 ...
	 *            The maximum size of the cache in bytes.
	 */
	public SelfDiskCache1(File rootDirectory, int maxCacheSizeInBytes) {
		mRootDirectory = rootDirectory;
		mMaxCacheSizeInBytes = maxCacheSizeInBytes;
		mBackupDirectory = rootDirectory;
	}

	public SelfDiskCache1(File rootDirectory, File backDirectory) {
		mRootDirectory = rootDirectory;
		mMaxCacheSizeInBytes = DEFAULT_DISK_USAGE_BYTES;
		mBackupDirectory = backDirectory;
	}
	 ...
	 *            The root directory of the cache.
	 */
	public SelfDiskCache1(File rootDirectory) {
		this(rootDirectory, DEFAULT_DISK_USAGE_BYTES);
	}

	/**
	 * 将头文件的image转换成正常的image
	 * 
	 * @param srcPath
	 * @param descPath
	 * @return
	 */
	public String getFileImage(String srcPath, String descPath) {
		String retValue = "";
		if (TextUtils.isEmpty(srcPath) || TextUtils.isEmpty(descPath)) {
			return retValue;
		}
	...
	public synchronized Entry get(String key) {
		...
		try {
			...
			// CacheHeader.readHeader(cis); // eat header
			byte[] data = streamToBytes(is, (int) file.length());
	...
	public synchronized void initialize() {
		...
		for (File file : files) {
			...
			if (name.endsWith(".jpg")) {//兼容之前的照片已“.jpg”结尾
				name = name.substring(0, name.length() - 4);
			}
			if (file.isDirectory()) {
				continue;
			}
			File backFile = new File(mBackupDirectory, name);
			FileInputStream fis = null;
			try {
				fis = new FileInputStream(backFile);
				CacheHeader entry = CacheHeader.readHeader(fis);
				entry.size = file.length() + backFile.length();
	...
	public synchronized void put(String key, Entry entry) {
		...
		File file = getFileForKey(key);
		File backFile = getFileBackForKey(key);
		try {
			FileOutputStream fos = new FileOutputStream(file);
			FileOutputStream backos = new FileOutputStream(backFile);
			CacheHeader e = new CacheHeader(key, entry);
			e.writeHeader(backos);
	 ...
	 * Returns a file object for the given cache key.
	 */
	public File getFileBackForKey(String key) {
		return new File(mBackupDirectory, getFilenameForKey(key));
	}
	...
	private void pruneIfNeeded(int neededSpace) {
		// 不要自动清除
		if (true || (mTotalSize + neededSpace) < mMaxCacheSizeInBytes) {
			return;
		}
		if (VolleyLog.DEBUG) {
