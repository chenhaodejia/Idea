	...
	protected void animateStack(View left, View right, float positionOffset, int positionOffsetPixels) {		
		if (mState != State.IDLE) {
			...
			if (left != null) {
				...
				mTrans = -getPageMargin() + positionOffsetPixels;
				ViewHelper.setScaleX(left, mScale);
				ViewHelper.setScaleY(left, mScale);
				ViewHelper.setTranslationX(left, mTrans * 0.8f);
			...
			// ViewHelper.setScaleX(right, mScale);
			// ViewHelper.setScaleY(right, mScale);
			// ViewHelper.setTranslationX(right, mTrans * 0.8f);
